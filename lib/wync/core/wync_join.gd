...
static func service_wync_try_to_connect(ctx: WyncCtx) -> int:
	...
	# throttle
	if WyncMisc.fast_modulus(ctx.common.ticks, 8) != 0:
		return OK
...
static func wync_handle_pkt_join_res(ctx: WyncCtx, data: Variant) -> int:
	...
	if not data.approved:
		Log.errc(ctx, "Connection DENIED for client(%s) (me)" % [ctx.common.my_nete_peer_id])
		return 2
	...
	ctx.common.my_peer_id = data.wync_client_id
	WyncJoin.client_setup_my_client(ctx, data.wync_client_id)

	Log.outc(ctx, "client nete_peer_id(%s) connected as wync_peer_id(%s)" % [ctx.common.my_nete_peer_id, ctx.common.my_peer_id])
	return OK
...
static func wync_handle_pkt_join_req(ctx: WyncCtx, data: Variant, from_nete_peer_id: int) -> int:
	...
	ctx.co_throttling.out_peer_pending_to_setup.append(from_nete_peer_id)
...
static func peer_register(ctx: WyncCtx, peer_data: int = -1) -> int:
	var peer_id = ctx.common.peers.size()
	ctx.common.peers.append(peer_data)
	ctx.co_clientauth.client_owns_prop[peer_id] = []
	ctx.co_track.client_has_relative_prop_has_last_tick[peer_id] = {}
	
	ctx.common.client_has_info[peer_id] = WyncCtx.WyncClientInfo.new()
...
static func is_peer_registered(ctx: WyncCtx, nete_peer_id: int) -> int:
	for peer_id: int in range(ctx.common.peers.size()):
		var i_peer_data = ctx.common.peers[peer_id]
...
static func wync_set_server_nete_peer_id (ctx: WyncCtx, nete_peer_id: int) -> int:
	if ctx.common.peers.size() == 0:
		ctx.common.peers.resize(1)
	ctx.common.peers[0] = nete_peer_id
	ctx.common.my_nete_peer_id = nete_peer_id
	return OK
...
static func get_nete_peer_id_from_wync_peer_id (ctx: WyncCtx, wync_peer_id: int) -> int:
	if wync_peer_id >= 0 && wync_peer_id < ctx.common.peers.size():
		return ctx.common.peers[wync_peer_id]
	return -1
...
static func client_setup_my_client(ctx: WyncCtx, peer_id: int) -> bool:
	...
	ctx.co_events.to_peers_i_sent_events.resize(1)
	ctx.co_events.to_peers_i_sent_events[ctx.SERVER_PEER_ID] = FIFOMap.new()
	ctx.co_events.to_peers_i_sent_events[ctx.SERVER_PEER_ID].init(ctx.common.max_amount_cache_events)

	# setup relative synchronization
	ctx.co_throttling.peers_events_to_sync = []
	ctx.co_throttling.peers_events_to_sync.resize(1)
	ctx.co_throttling.peers_events_to_sync[ctx.SERVER_PEER_ID] = {} as Dictionary
...
static func clear_peers_pending_to_setup(ctx: WyncCtx):
	ctx.co_throttling.out_peer_pending_to_setup.clear()


static func out_client_just_connected_to_server(ctx: WyncCtx) -> bool:
	var just_connected: bool = ctx.common.connected && not ctx.common.prev_connected
