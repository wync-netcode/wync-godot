...
func _init() -> void:
	...
	co_spawn = CoSpawn.new()

	## Server only

	co_throttling = CoThrottling.new()
	co_filter_s = CoFilterServer.new()

	## Client only

	co_ticks = CoTicks.new()
	co_pred = CoPredictionData.new()
	co_lerp = CoLerp.new()
	co_dummy = CoDummyProps.new()
	co_filter_c = CoFilterClient.new()


	CoTicksUtils.init_co_ticks(co_ticks)
	CoTicksUtils.init_co_prediction_data(co_pred)

	co_track.props.resize(MAX_PROPS)
	co_track.prop_id_cursor = 0
	co_track.active_prop_ids = []

	co_events.peer_has_channel_has_events.resize(common.max_peers)
	co_events.prop_id_by_peer_by_channel.resize(common.max_peers)
	co_track.client_has_relative_prop_has_last_tick.resize(common.max_peers) # NOTE: index 0 not used
	co_spawn.out_queue_spawn_events = FIFORingAny.new(1024)

	co_throttling.queue_clients_entities_to_sync.resize(common.max_peers)
	co_throttling.queue_entity_pairs_to_sync.resize(100)

	co_throttling.clients_sees_entities.resize(common.max_peers)
	co_throttling.clients_sees_new_entities.resize(common.max_peers)
	co_throttling.clients_no_longer_sees_entities.resize(common.max_peers)
	co_throttling.entities_synced_last_time.resize(common.max_peers)

	co_throttling.clients_cached_unreliable_snapshots.resize(common.max_peers)
	co_throttling.clients_cached_reliable_snapshots.resize(common.max_peers)

	common.peer_latency_info.resize(common.max_peers)

	for peer_i in range(common.max_peers):
		co_events.peer_has_channel_has_events[peer_i] = []
		co_events.peer_has_channel_has_events[peer_i].resize(common.max_channels)
		for channel_i in range(common.max_channels):
			co_events.peer_has_channel_has_events[peer_i][channel_i] = []
		co_track.client_has_relative_prop_has_last_tick[peer_i] = {}

		co_events.prop_id_by_peer_by_channel[peer_i] = []
		co_events.prop_id_by_peer_by_channel[peer_i].resize(common.max_channels)
		for channel_i in range(common.max_channels):
			co_events.prop_id_by_peer_by_channel[peer_i][channel_i] = -1

		#if peer_i != WyncCtx.SERVER_PEER_ID:
		co_throttling.queue_clients_entities_to_sync[peer_i] = FIFORing.new()
		co_throttling.queue_clients_entities_to_sync[peer_i].init(128) # TODO: Make this user defined
		co_throttling.clients_sees_entities[peer_i] = {}
		co_throttling.clients_sees_new_entities[peer_i] = {}
		co_throttling.clients_no_longer_sees_entities[peer_i] = {}
		co_throttling.entities_synced_last_time[peer_i] = {}

		var latency_info = WyncCtx.PeerLatencyInfo.new()
		latency_info.latency_buffer.resize(WyncCtx.LATENCY_BUFFER_SIZE)
		common.peer_latency_info[peer_i] = latency_info

		co_throttling.clients_cached_reliable_snapshots[peer_i] = [] as Array[WyncPktSnap.SnapProp]
		co_throttling.clients_cached_unreliable_snapshots[peer_i] = [] as Array[WyncPktSnap.SnapProp]
	
	co_pred.tick_action_history = RingBuffer.new(co_pred.tick_action_history_size, {})
	for i in range(co_pred.tick_action_history_size):
		co_pred.tick_action_history.insert_at(i, {} as Dictionary)
	
	common.client_has_info.resize(common.max_peers)

	co_metrics.debug_packets_received.resize(WyncPacket.WYNC_PKT_AMOUNT)
	for i in range(WyncPacket.WYNC_PKT_AMOUNT):
		co_metrics.debug_packets_received[i] = [] as Array[int]
		co_metrics.debug_packets_received[i].resize(20) # amount of co_track.props, also 0 is reserved for 'total'

	co_metrics.debug_data_per_tick_sliding_window = RingBuffer.new(co_metrics.debug_data_per_tick_sliding_window_size, 0)

	co_metrics.server_tick_rate_sliding_window = RingBuffer.new(SERVER_TICK_RATE_SLIDING_WINDOW_SIZE, 0)

	co_metrics.low_priority_entity_update_rate_sliding_window = RingBuffer.new(co_metrics.low_priority_entity_update_rate_sliding_window_size, 0)

	co_dummy.dummy_props = {}

	co_lerp.max_lerp_factor_symmetric = 1.0
