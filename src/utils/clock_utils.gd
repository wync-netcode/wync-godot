...
static func get_predicted_tick_local_time_msec(ticks: int, co_ticks: CoTicks, co_predict_data: CoPredictionData) -> int:
...
"""
static func local_time_to_server_time(co_predict_data: CoPredictionData, local_time: int) -> int:
	return local_time + co_predict_data.clock_offset_mean


static func server_time_to_local_time(co_predict_data: CoPredictionData, server_time: int) -> int:
...
static func get_tick_local_time_msec(co_predict_data: CoPredictionData, co_ticks: CoTicks, ticks: int) -> int:
	var frame = 1000.0 / Engine.physics_ticks_per_second
