...
func on_process(entities, _data, _delta: float):
	...
	var target_time = curr_time + co_loopback.lag + (1000.0 / 30)

	#var target_tick = co_predict_data.last_tick_confirmed + ceil((target_time - curr_time) / tick_rate)
	var target_tick = co_predict_data.last_tick_confirmed + ceil((target_time - co_predict_data.last_tick_timestamp) / float(1000.0 / 30))
	...
	for entity: Entity in entities:
		...
		for tick in range(curr_predicted.tick +1, target_tick +1):
			...
			prev_predicted.timestamp += 1000.0 / 30
			curr_predicted.timestamp += 1000.0 / 30
